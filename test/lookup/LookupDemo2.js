const {
  ...
  describe("LookupDemo2", function () {
    // We define a fixture to reuse the same setup in every test.
    // We use loadFixture to run this setup once, snapshot that state,
    // and reset Hardhat Network to that snapshot in every test.
    async function deployLookupDemo2Fixture() {
      const [owner, otherAccount] = await ethers.getSigners();
  
      const LookupDemo2 = await ethers.getContractFactory("LookupDemo2");
      const lookup = await LookupDemo2.deploy();
      const NUMBERS = [
    ...
    describe("contract", function () {
      it("Should return correct results", async function () {
        const { lookup, NUMBERS } = await loadFixture(deployLookupDemo2Fixture);
        let res = []
