const {
  ...
  describe("MaskingDemo1", function () {
    // We define a fixture to reuse the same setup in every test.
    // We use loadFixture to run this setup once, snapshot that state,
    // and reset Hardhat Network to that snapshot in every test.
    async function deployMaskingDemo1Fixture() {
      const [owner, otherAccount] = await ethers.getSigners();
  
      const MaskingDemo1 = await ethers.getContractFactory("MaskingDemo1");
      const maskingD = await MaskingDemo1.deploy();
    ...
    describe("contract - deployer", function () {
      it("should return true when owner calls callerIsStoredPlainSolidity()", async function () {
        const { maskingD, owner } = await loadFixture(deployMaskingDemo1Fixture);
    ...
    describe("contract - non deployer", function () {
        it("should return false when non deployer calls callerIsStoredPlainSolidity()", async function () {
          const { maskingD, otherAccount } = await loadFixture(deployMaskingDemo1Fixture);
